...
class PairingManager:
    ...
    async def system_pair(self, address: str) -> bool:
        ...
        if system == "Linux":
            ...
            return await self._pair_macos(address)
        elif system == "Windows":
    ...
    async def _pair_macos(self, address: str) -> bool:
        """Pair on macOS using blueutil or CoreBluetooth
        
        macOS handles pairing differently:
        1. For BLE devices, pairing often happens automatically during connection
        2. Some devices require explicit pairing through System Preferences
        3. Can use blueutil (if installed) or AppleScript for automation
        """
        try:
            import subprocess
            
            # Method 1: Try using blueutil if installed
            try:
                ...
                if result.returncode == 0:
                    # Use blueutil to pair
                    logger.info("Using blueutil for pairing on macOS")
                    
                    # Connect to device (which triggers pairing on macOS)
                    result = subprocess.run(
                    ...
                    if result.returncode == 0:
                        logger.info(f"Successfully paired with {address} using blueutil")
                        return True
                    else:
                        logger.warning(f"blueutil pairing failed: {result.stderr}")
            except:
                pass
            
            # Method 2: Use AppleScript to trigger pairing through System Preferences
            applescript = f'''
            ...
            tell application "System Events"
                tell process "System Preferences"
                    ...
                    tell window "Bluetooth"
                        -- This would need more sophisticated handling for real pairing
                        -- For now, we'll just open the Bluetooth preferences
                    end tell
                end tell
            end tell
            '''
            
            # Method 3: For BLE devices, pairing typically happens automatically
            # during the first connection attempt via CoreBluetooth
            logger.info("macOS: BLE pairing will be handled automatically during connection")
            logger.info("If pairing fails, please pair manually through System Preferences > Bluetooth")
            
            # For BLE devices on macOS, we return True because pairing
            # happens automatically when using CoreBluetooth/bleak
            return True
            
        except Exception as e:
            logger.error(f"macOS pairing error: {e}")
            logger.info("Please pair the device manually through System Preferences > Bluetooth")
            return True  # Return True for BLE devices as they auto-pair
